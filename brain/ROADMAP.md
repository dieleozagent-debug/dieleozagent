...
| **D7** | **🆕 Bug `resetOracle()` no funciona dentro del container del agente** | `src/sapi/notebooklm_mcp.js:resetOracle()` ejecuta `docker restart notebooklm-mcp-v12` pero el container `dieleozagent-debug-dieleozagent-1` NO tiene binario `docker` ni acceso al socket del host. El reset falla silenciosamente con `/bin/sh: 1: docker: not found`. Mitigación parcial v14.8.8: subir `MCP_QUERY_TIMEOUT_MS` de 90s a 240s para reducir falsos atascos. Si Chrome se atasca de verdad, el operador debe reiniciar manualmente con `docker restart notebooklm-mcp-v12` desde el host. **Confirmado en producción:** auditoría /audit fibra 2026-05-08 21:02 disparó "Chrome atascado", el reset falló silenciosamente, y la Fase 3 (Juez) ejecutó SIN respuesta del Oráculo — el Juez aprobó solo con SSoT (RAG con BCD V001), sin contrastar con NotebookLM. | Solución: (a) montar docker socket `/var/run/docker.sock` al container del agente; o (b) usar API REST del docker engine; o (c) sustituir `resetOracle()` por una llamada al MCP server pidiendo `reset session`. Sin Vo.Bo. del Director Técnico UF2 no se decide cuál. **Workaround inmediato:** restart manual desde host antes de cada `/audit`. |
